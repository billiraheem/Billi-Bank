	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					DepositTx(gomock.Any(), gomock.Eq(args)).
					Times(1).
					Return(db.DepositTxResult{
						Account: account,
						Entry: db.Entry{
							AccountID: account.ID,
							Amount:    10,
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				...
				// read the body and check it contains the account
				data, err := io.ReadAll(recorder.Body)
				require.NoError(t, err)

				var gotResult db.DepositTxResult
				err = json.Unmarshal(data, &gotResult)
				require.NoError(t, err)
				require.Equal(t, account, gotResult.Account)
			},
		},
		{
			name:      "NotFound",
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					DepositTx(gomock.Any(), gomock.Eq(args)).
					Times(1).
					Return(db.DepositTxResult{}, sql.ErrNoRows)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusNotFound, recorder.Code)
			},
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					DepositTx(gomock.Any(), gomock.Eq(args)).
					Times(1).
					Return(db.DepositTxResult{}, sql.ErrConnDone)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusInternalServerError, recorder.Code)
			},
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					DepositTx(gomock.Any(), gomock.Any()).
					Times(0)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusBadRequest, recorder.Code)
